 ...
 *
 * Everything about the batch lives in a single collapsible card: the title and
 * status badge, the two controls that act on it, its identifying facts, and the
 * exact question list. The card starts collapsed because the list is the long
 * part and the reason to open a batch is usually to change its status or run it,
 * not to read every stem.
 *
 * What stays visible while collapsed is deliberate: an unresolved ID and a
 * failed Practice launch are reported outside the collapsed body, so a disabled
 * Practice button is never unexplained. The list itself is rendered from
 * `batch.questionIds` in stored order, not from a re-query of the catalog, so
 * what is shown is what a Practice run or an export will contain. An id that no
 * longer resolves blocks both, because a batch that silently shrinks to the
 * items that happen to still exist is the kind of quiet inaccuracy this project
 * refuses.
 */
function BatchWorkspace({ batch, state }: { batch: RefinementBatch; state: RefinementBatchesState }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
      ...
      <section
        aria-labelledby="batch-card-heading"
      ...
      >
        <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 flex-wrap items-center gap-3">
            <h1 id="batch-card-heading" className="min-w-0">
              <button
                type="button"
                onClick={() => setExpanded((current) => !current)}
                aria-expanded={expanded}
                aria-controls="batch-card-details"
                className="inline-flex min-h-11 items-center gap-2 rounded-lg px-1 text-left text-2xl font-extrabold text-slate-900 transition hover:text-emerald-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:text-white dark:hover:text-emerald-400"
              >
                {expanded ? (
                ...
                <span className="min-w-0">{batch.title}</span>
              </button>
            </h1>
          ...
          <div className="flex shrink-0 flex-col gap-1.5 lg:items-end">
            ...
            <p className="text-[11px] text-slate-500 lg:text-right dark:text-slate-400">
              Practice runs are not recorded in history or analytics.
            </p>
          </div>
        </div>

        {(practiceError || missingIds.length > 0) && (
        ...
        {expanded && (
          <div id="batch-card-details" className="border-t border-slate-200 p-5 dark:border-slate-800">
            ...
            <section
              ...
              className="mt-5 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800"
            >
              <div className="flex flex-wrap items-end justify-between gap-2 border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                <h2
        ...
        >
          ...
          {loading && !error ? (
            ...
            <p
              ...
              className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-300"
            >
              <strong>Review Markdown, Raw JSON, and Question Set are unavailable.</strong> These IDs are not in the active question bank:{' '}
