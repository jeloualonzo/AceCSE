      ...
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onRetake}
              className="inline-flex items-center gap-2 px-4 py-2 min-h-[44px] rounded-xl text-xs sm:text-sm font-semibold bg-slate-800 text-slate-200 hover:bg-slate-700 hover:text-white border border-slate-700 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
            >
              <RotateCcw className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              <span>Retake</span>
            </button>
            <button
            ...
            <div className="flex items-center gap-5 text-center sm:text-left">
              ...
              <div>
                {isSimulation && (
                  <div className="mb-2">
                    {attempt.passed ? (
                      <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-2.5 py-0.5 rounded-full inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wide">
                        <CheckCircle2 className="w-3.5 h-3.5" aria-hidden="true" /> Passed
                      </span>
                    ) : (
                      <span className="bg-rose-500/20 text-rose-300 border border-rose-500/40 px-2.5 py-0.5 rounded-full inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wide">
              ...
              >
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  ...
                  <span className="font-mono font-bold text-slate-300">
                    ...
                    <span className="text-slate-500">({subject.percentage.toFixed(0)}%)</span>
                  </span>
                </div>
                <div
        ...
        <div className="space-y-4 pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h3 className="text-lg font-bold text-white">Item Review</h3>
            ...
            <div className="space-y-4">
              {filteredItems.map(({ item, number }) => {
                ...
                return (
                  <div
                    key={item.questionId}
                    className={`bg-slate-900 border rounded-2xl p-5 sm:p-6 shadow-sm ${
                      item.isCorrect
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-3">
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-white p-2 min-h-[40px] min-w-[40px] rounded hover:bg-slate-800 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
                    ...
                    {isExpanded && (
                      <>
                        {question.passage && (
                          <div className="mb-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700/60 text-xs sm:text-sm text-slate-300 whitespace-pre-line">
                            {question.passage}
                          </div>
                        )}
                        <div className="space-y-2 mb-4">
                          {question.choices.map((option) => {
                            ...
                            if (isCorrectOption) {
                              optionStyle =
                                'bg-emerald-950/60 border-emerald-500/80 text-emerald-100 font-semibold';
                            } else if (isUserPick) {
                              optionStyle =
                                'bg-rose-950/60 border-rose-500/80 text-rose-100 font-semibold';
                            }
                            return (
                              <div
                                key={option.id}
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 shrink-0 text-[11px] font-bold">
                                  {isCorrectOption && (
                                    <span className="text-emerald-400 bg-emerald-900/50 px-2 py-0.5 rounded border border-emerald-500/30">
                                      Correct answer
                                    </span>
                                  )}
                                  {isUserPick && (
                                    <span
                                      className={`px-2 py-0.5 rounded border ${
                                        isCorrectOption
                                          ? 'text-emerald-400 bg-emerald-900/50 border-emerald-500/30'
                                          : 'text-rose-400 bg-rose-900/50 border-rose-500/30'
                                      }`}
                                    >
                                      Your choice
                                    </span>
